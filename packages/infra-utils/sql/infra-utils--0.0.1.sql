\echo Use "CREATE EXTENSION infra-utils" to load this file. \quit
CREATE SCHEMA infra_utils;

GRANT USAGE ON SCHEMA infra_utils TO PUBLIC;

ALTER DEFAULT PRIVILEGES IN SCHEMA infra_utils
  GRANT EXECUTE ON FUNCTIONS TO PUBLIC;

CREATE FUNCTION infra_utils.quantity_to_numeric(
  quantity text
) RETURNS numeric AS $EOFCODE$
DECLARE
  trimmed text;
  digits text;
  suffix text;
...
CREATE FUNCTION infra_utils.validate_params_schema(
  params_schema jsonb
...
CREATE FUNCTION infra_utils.bundle_param_interface(
  schemas jsonb
) RETURNS jsonb AS $EOFCODE$
DECLARE
  conflict record;
