-- Deploy schemas/metaschema_public/tables/table_behavior/table to pg
...
-- One row per PostGraphile v5 behavior fragment attached to a table. An
...
-- fragment per scope: writers upsert on (table_id, scope) rather than appending a
-- second fragment that would silently shadow the first.
CREATE TABLE metaschema_public.table_behavior (
  id uuid PRIMARY KEY DEFAULT uuidv7(),
  database_id uuid NOT NULL DEFAULT uuid_nil(),

  table_id uuid NOT NULL REFERENCES metaschema_public.table (id) ON DELETE CASCADE,
  ...
  CONSTRAINT table_behavior_modifier_check CHECK (modifier IN ('+', '-')),

  CONSTRAINT table_behavior_scope_check CHECK (
    scope ~ '^([a-zA-Z][a-zA-Z0-9]*|\*)(:([a-zA-Z][a-zA-Z0-9]*|\*))*$'
  ),

  CONSTRAINT table_behavior_scope_key UNIQUE (table_id, scope)
);
