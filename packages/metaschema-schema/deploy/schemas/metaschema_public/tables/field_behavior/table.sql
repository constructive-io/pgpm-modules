-- Deploy schemas/metaschema_public/tables/field_behavior/table to pg
...
-- One row per PostGraphile v5 behavior fragment attached to a field. An
...
-- fragment per scope: writers upsert on (field_id, scope) rather than appending a
-- second fragment that would silently shadow the first.
CREATE TABLE metaschema_public.field_behavior (
  id uuid PRIMARY KEY DEFAULT uuidv7(),
  database_id uuid NOT NULL DEFAULT uuid_nil(),

  field_id uuid NOT NULL REFERENCES metaschema_public.field (id) ON DELETE CASCADE,
  ...
  CONSTRAINT field_behavior_modifier_check CHECK (modifier IN ('+', '-')),

  CONSTRAINT field_behavior_scope_check CHECK (
    scope ~ '^([a-zA-Z][a-zA-Z0-9]*|\*)(:([a-zA-Z][a-zA-Z0-9]*|\*))*$'
  ),

  CONSTRAINT field_behavior_scope_key UNIQUE (field_id, scope)
);
