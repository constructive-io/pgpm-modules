...
INSERT INTO metaschema_public.node_type_registry (
  ...
  '{"type":"object","properties":{"limit_name":{"type":"string","description":"Name of the aggregate limit to watch (must match a limit_warnings.name entry, e.g. \"databases\", \"members\")"},"entity_field":{"type":"string","format":"column-ref","description":"Column on the target table that holds the entity id for aggregate limit lookup","default":"entity_id"}},"required":["limit_name"]}'::jsonb,
...
INSERT INTO metaschema_public.node_type_registry (
  ...
  '{"type":"object","properties":{"limit_name":{"type":"string","description":"Name of the limit to watch (must match a limit_warnings.name entry, e.g. \"projects\", \"members\")"},"scope":{"type":"string","enum":["app","org"],"description":"Limit scope: \"app\" (membership_type=1, user-level) or \"org\" (membership_type=2, entity-level)","default":"app"},"actor_field":{"type":"string","format":"column-ref","description":"Column on the target table that holds the actor id for limit lookup","default":"owner_id"}},"required":["limit_name"]}'::jsonb,
...
INSERT INTO metaschema_public.node_type_registry (
  ...
  '{"type":"object","properties":{"meter_slug":{"type":"string","description":"Slug of the billing meter to check rate limits against (must match a meters table entry)"},"entity_field":{"type":"string","format":"column-ref","description":"Column on the target table that holds the entity id for rate limit lookup","default":"entity_id"},"actor_field":{"type":"string","format":"column-ref","description":"Column on the target table that holds the actor id for rate limit lookup","default":"owner_id"}},"required":["meter_slug"]}'::jsonb,
