-- Deploy schemas/metaschema_public/tables/foreign_key_constraint_behavior/table to pg

-- requires: schemas/metaschema_public/schema
-- requires: schemas/metaschema_public/tables/database/table
-- requires: schemas/metaschema_public/tables/foreign_key_constraint/table

BEGIN;

-- One row per PostGraphile v5 behavior fragment attached to a foreign key constraint. An
...
-- fragment per scope: writers upsert on (foreign_key_constraint_id, scope) rather than appending a
...
CREATE TABLE metaschema_public.foreign_key_constraint_behavior (
  ...
  foreign_key_constraint_id uuid NOT NULL REFERENCES metaschema_public.foreign_key_constraint (id) ON DELETE CASCADE,
  ...
  CONSTRAINT foreign_key_constraint_behavior_modifier_check CHECK (modifier IN ('+', '-')),

  CONSTRAINT foreign_key_constraint_behavior_scope_check CHECK (
    scope ~ '^([a-zA-Z][a-zA-Z0-9]*|\*)(:([a-zA-Z][a-zA-Z0-9]*|\*))*$'
  ),

  CONSTRAINT foreign_key_constraint_behavior_scope_key UNIQUE (foreign_key_constraint_id, scope)
);
