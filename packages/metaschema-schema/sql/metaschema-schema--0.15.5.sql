...
INSERT INTO metaschema_public.node_type_registry (
  ...
  ('TableUserProfiles', 'table_user_profiles', 'data', 'User Profiles', 'Creates a user profiles table with standard profile fields (profile_picture, bio, first_name, last_name, tags, desired). Uses AuthzDirectOwner for edit access and AuthzAllowAll for select.', '{"type":"object","properties":{}}'::jsonb, CAST('{"template","settings","ownership","schema"}' AS text[])) ON CONFLICT (name) DO UPDATE SET 
  slug = excluded.slug,
  category = excluded.category,
  display_name = excluded.display_name,
  description = excluded.description,
...
INSERT INTO metaschema_public.node_type_registry (
  ...
  ('TableUserSettings', 'table_user_settings', 'data', 'User Settings', 'Creates a user settings table for user-specific configuration. Uses AuthzDirectOwner for access control.', '{"type":"object","properties":{}}'::jsonb, CAST('{"template","settings","ownership","schema"}' AS text[])) ON CONFLICT (name) DO UPDATE SET 
  slug = excluded.slug,
  category = excluded.category,
  display_name = excluded.display_name,
  description = excluded.description,
